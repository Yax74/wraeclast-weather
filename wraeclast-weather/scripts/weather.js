...
Hooks.once('init', () => {
  ...
  game.settings.register("wraeclast-weather", "useSimpleCalendar", {
    ...
    hint: "If enabled, will pull the current month from Simple Calendar.",
    scope: "world",
    config: true,
    type: Boolean,
    default: false
...
const seasonalRolls = {
  "Derivi": "2d5+6", "Phreci": "2d5+5", "Caspiri": "2d5+4", "Astrali": "3d3+1", "Eterni": "2d3+1", "Atziri": "3d3+1",
  "Vivici": "2d5+3", "Lurici": "2d6+2", "Sagari": "2d5+4", "Vitali": "2d5+5", "Azmeri": "2d5+6", "Verusi": "2d6+6", "Divini": "2d5+7"
};
...
const windTable = [
  { roll: 2, wind: "Calm" }, { roll: 5, wind: "Gentle Breeze" }, { roll: 8, wind: "Moderate Wind" },
  { roll: 11, wind: "Gusty" }, { roll: 13, wind: "Windy" }, { roll: 15, wind: "Strong Wind" },
  { roll: 17, wind: "Strong Gusts" }, { roll: 19, wind: "Severe Wind" }, { roll: 20, wind: "Dangerous Winds" }
];

// Main macro execution
Hooks.on("ready", async () => {
  game.wraeclastWeather = async function generateWeather() {
    const useCalendar = game.settings.get("wraeclast-weather", "useSimpleCalendar");

    let currentMonth = game.settings.get("wraeclast-weather", "defaultSeason");
    if (useCalendar && game.modules.get("foundryvtt-simple-calendar")?.active && game?.SimpleCalendar?.api) {
      const date = game.SimpleCalendar.api.currentDate();
      currentMonth = date.month.name;
    } else {
      const months = Object.keys(seasonalRolls);
    ...
    const selectedTerrain = await new Promise(resolve => {
      new Dialog({
    ...
    const terrainMod = terrainModifiers[selectedTerrain] || 0;
    let seasonalDiceRoll = await new Roll(seasonalRolls[currentMonth]).roll({ async: true });
    let adjustedTempRoll = seasonalDiceRoll.total + terrainMod;
    ...
    ChatMessage.create({
      content: `<strong>Today's Weather:</strong><br>
        ...
        <hr><small>Month: ${currentMonth} | Terrain: ${selectedTerrain}</small>`
    });
  };
});
